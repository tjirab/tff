"""Orchestrator runner executing tff-core rules and checks against Dataform models."""
...
def collect_dataform_rules_findings(models: dict[str, ModelRepresentation]) -> list[LintFinding]:
    findings = []
...
def run_all_checks(
    ...
    if "rules" in selected:
        findings.extend(collect_dataform_rules_findings(models))
    ...
    models_checked = sum(1 for m in models.values() if not m.is_external and not m.is_symbolic)
